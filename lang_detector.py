...
def evaluate_model_custom(model, X_test, y_test):
    ...
    sums_y = np.sum(cm, axis=0)
    sums_x = np.sum(cm, axis=1)

    for i in range(len(cm)):
        tp = cm[i][i]
        precisions.append(tp / (sums_y[i]))
        recalls.append(tp / (sums_x[i]))
        f1_scores.append(2 * (1 / (1 / precisions[i] + 1 / recalls[i])))

    print("lang\tprecision\trecall\t\tf-1 score")
    for i in range(len(cm)):
        print(
            "{}\t{:.6f}\t{:.6f}\t{:.6f}".format(
                label2lang[i], precisions[i], recalls[i], f1_scores[i]
            )
        )

    macro_f1 = np.mean(f1_scores)
    # micro averaging essentially computes the proportion of correctly
    # classified observations out of all observations -> that is accuracy
    # in general in multi-class classification, micro-F1 = accuracy
    micro_f1 = accuracy
...
def compute_confusion_matrix(y_test, y_pred):
    array = np.arange(len(languages) * len(languages)).reshape(
        (len(languages), len(languages))
    )
    matrix = np.zeros_like(array, dtype=float)
...
def load_data(max_examples_per_lang=1000):
    ...
    df = pd.concat(
        (
            pd.read_csv(
                os.path.join(data_path, lang + ".tsv"),
                names=col_names,
                encoding="utf-8",
                sep="\t",
                nrows=max_examples_per_lang,
            )
    ...
    ### START CODE HERE ### (≈ 1 line of code)
    df["label"] = df["label"].map(lambda value: lang2label[value])
    ### END CODE HERE ###
